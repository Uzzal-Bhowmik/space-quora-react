...
const WorldMap = () => {
    ...
    useEffect(() => {
        ...
        map.on('mousemove', (e) => {
            ...
            tooltip.setContent(`Latitude: ${lat.toFixed(4)}, Longitude: ${lng.toFixed(4)}`);
        });
    ...
    useEffect(() => {
        { weatherData && toast.success(`Searching for: ${weatherData?.name}, ${weatherData?.sys?.country}`) }
    }, [weatherData])
    ...
    return (
        <div>
            ...
            {selectedLocation && (
                <ClimatePredictionChart weatherData={weatherData} latitude={selectedLocation.lat} longitude={selectedLocation.lng} />
            )}
        </div>
    );
};
